...
/**
 * Datafixtures for the Languages.
 *
 ...
 */
class LoadLanguageData extends AbstractFixture implements ContainerAwareInterface, OrderedFixtureInterface {
    ...
    public function load(ObjectManager $manager) {
        ...
        $path = $kernel->locateResource(
            '@TheaterjobsProfileBundle/DataFixtures/SQL/languages.csv'
        );
        $skills = $this->csvToArray($path);

        $console = new ConsoleOutput();
        $console->writeln("Ok,will take some time to load " . count($skills) . " languages");

        foreach ($skills as $value) {
            $skill = new Skill();
            $skill->setTitle($value[0]);
            $skill->setChecked(1);
            $skill->setIsLanguage(1);
